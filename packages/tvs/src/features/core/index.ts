...
export function createTvs(options?: TvsOptions) {
  ...
  function customTvs(base: string): TvsBuilder<Record<never, never>, Record<never, never>> {
    return makeBuilder(
      base,
      {} as Record<never, never>,
      {} as Record<never, never>,
  ...
  function customCn(...args: ClassValue[]): string {
    let str = "";
    for (let i = 0; i < args.length; i++) {
      const val = toVal(args[i]);
      if (val) {
  ...
  return { tvs: customTvs, cn: customCn };
}
